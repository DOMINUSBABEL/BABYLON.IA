...
**Vulnerability:** The functions in `src/jules_bridge.js` used `child_process.exec` with string interpolation for executing `jules` remote commands. User-supplied inputs (`prompt` and `sessionId`) were injected directly into the shell string, allowing for command injection.
**Learning:** Using `child_process.exec` with user inputs allows the shell to interpret shell metacharacters and execute arbitrary commands.
**Prevention:** Always use `child_process.execFile` (or `spawn` without `{shell: true}`) and pass arguments as an explicit array. This bypasses the shell entirely, ensuring that inputs are treated purely as arguments, regardless of their content.
