...
## 2025-05-24 - [Missing HTTP Security Headers]
**Vulnerability:** The Express server in `src/server.js` was serving static files and handling API requests without any HTTP security headers. This lack of defense-in-depth exposed the application to various client-side attacks, including Cross-Site Scripting (XSS), Clickjacking, and MIME sniffing.
**Learning:** Security headers are a fundamental layer of defense for web applications. Their absence is a surprising security gap in an architecture that otherwise implements several advanced security features. Relying solely on client-side sanitization is insufficient; server-side headers must be used to mitigate the impact of potential vulnerabilities.
**Prevention:** Implemented a custom middleware in Express before serving any content to set `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, and `X-XSS-Protection: 1; mode=block`.
