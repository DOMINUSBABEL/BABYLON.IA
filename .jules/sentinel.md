...
## 2025-06-05 - [XSS and Command Injection Fixes]
**Vulnerability:** Found `innerHTML` being set with potentially unsafe content via `appendLog` and `appendReasoning` in `public/app.js`. Found `child_process.exec` being used to call `ollama list` in `src/server.js`, and `open` in `src/auth.js`. Finally, found `onclick` event handlers being formatted dynamically without escaping paths in `public/app.js`, potentially allowing an attacker to escape string boundaries or inject JavaScript.
**Learning:** `innerHTML` allows the execution of XSS payloads. Always sanitize or escape any inputs before concatenating them with raw HTML. Also avoid passing any dynamic execution strings to `child_process.exec` as those are passed straight to the shell and can lead to command injection. Use `child_process.execFile` instead. Always escape paths when evaluating them to be used in HTML element attributes like `onclick` or `title`.
**Prevention:** Ensured all occurrences of `child_process.exec` are changed to `child_process.execFile`. Implemented an `escapeHTML` helper function in `public/app.js` and modified all relevant code that modifies `innerHTML` to use the helper. Used `replace(/'/g, "\\'")` to escape single quotes, then `escapeHTML` on paths being fed to HTML strings.
