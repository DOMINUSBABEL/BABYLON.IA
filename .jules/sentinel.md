...
## 2025-05-24 - [Path Traversal in LLM Tool Action Input]
**Vulnerability:** The `read_local_file` action in `src/tools_registry.js` accepted user-provided (or LLM-provided) paths (`actionInput`) and resolved them directly without checking directory boundaries, allowing indirect prompt injection to read arbitrary system files (e.g., `../../../../etc/passwd`).
**Learning:** Tool actions used by LLMs or external inputs must always validate paths to ensure they stay within the intended sandbox or workspace directory, specifically enforcing boundaries with path separators to prevent prefix-matching traversals.
**Prevention:** Implemented strict directory boundary validation in `executeTool` for the `read_local_file` action, ensuring `resolvedPath` strictly starts with `process.cwd() + path.sep` or strictly equals `process.cwd()`.
