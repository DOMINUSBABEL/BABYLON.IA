...
## 2025-02-14 - Path Traversal Vulnerability in LLM Tool Action Inputs
**Vulnerability:** The `read_local_file` tool action in `src/tools_registry.js` allowed directory traversal because the user input path (from an LLM parameter) was blindly joined using `path.resolve(process.cwd(), actionInput)` and read using `fs.readFileSync`. This allows the LLM or a user exploiting indirect prompt injection to read arbitrary files from the filesystem (like `/etc/passwd` or `.env`) using inputs like `../../../etc/passwd`.
**Learning:** LLM tool action parameters must be treated as untrusted input just like web requests, and require strict security validation (such as directory boundary checks) before accessing system resources like the filesystem or executing commands.
**Prevention:** Always validate resolved paths against an allowed base directory by ensuring the `resolvedPath` strictly matches the base path or starts with `basePath + path.sep`. Avoid using `path.resolve` alone as a security measure, as it successfully resolves `../` tokens to parent directories outside the intended bounds.
