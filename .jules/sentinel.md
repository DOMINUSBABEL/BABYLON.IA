...
**Vulnerability:** The `read_local_file` tool in `src/tools_registry.js` directly used user-provided or LLM-generated inputs to resolve paths relative to `process.cwd()`. This permitted a malicious prompt or indirect prompt injection (e.g. from an analyzed file) to coerce the agent into reading arbitrary system files like `/etc/passwd`.
**Learning:** Tool action inputs must be treated as fully untrusted. Even if an LLM is assumed to follow instructions, indirect prompt injections can force the tool to read outside intended directory boundaries.
**Prevention:** Implemented strict directory boundary validation in `read_local_file` by checking if the absolute resolved path explicitly matches or correctly starts with the base directory (`baseDir + path.sep`).
