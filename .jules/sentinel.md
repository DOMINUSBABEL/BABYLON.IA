...
## 2025-05-24 - [Command Injection via shell execution with string interpolation in agent_core.js]
**Vulnerability:** In `src/agent_core.js`, when executing the Gemini CLI on Windows (`process.platform === 'win32'`), `child_process.spawn` was called with `shell: true` and the arguments were passed via string interpolation (`geminiProcess = spawn(`${geminiBin} -m ${activeModel} -p . -o json`, { shell: true });`). This allowed for severe command injection if `activeModel` or other variables were maliciously crafted and bypassed validation.
**Learning:** String interpolation should never be used inside `spawn` or `exec` commands, especially when `shell: true` is enabled. Instead, passing an array of arguments delegates the tokenization to Node.js and avoids simple injection vectors (even if validation is present, this provides defense-in-depth).
**Prevention:** Replaced the string template interpolation with an array of arguments passed to `spawn`, adhering to secure coding practices by cleanly separating the executable from its parameters.
