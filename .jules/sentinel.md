...
## 2025-05-24 - [XSS Vulnerability in Web Dashboard]
**Vulnerability:** The web dashboard (`public/app.js`) dynamically created HTML by concatenating strings using `innerHTML`, taking untrusted data such as websocket logs, reasoning strings, and file names/paths without sanitization. This allowed for Cross-Site Scripting (XSS). In particular, when attempting to inject a file path into an inline JavaScript attribute (`onclick`), the initial escaping attempt (`.replace(/'/g, "\\'")`) was rendered useless if the HTML was not also escaped appropriately or if it was escaped in the wrong order.
**Learning:** All untrusted data rendered into the DOM via `innerHTML` must be strictly sanitized using an HTML escaping utility. When injecting data into an inline JavaScript attribute (e.g. `onclick="foo('...')"`), the string must *first* have its JavaScript string delimiters escaped (e.g. escaping single quotes), and *then* the entire string must be HTML-escaped. The browser decodes HTML entities in attributes before passing the value to the JavaScript engine.
**Prevention:** Introduced an `escapeHTML` utility in `public/app.js` to systematically sanitize incoming data before DOM insertion. For inline JS event handlers, applied the correct escaping sequence: `escapeHTML(item.path.replace(/'/g, "\\'"))`.
