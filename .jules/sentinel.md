...
**Vulnerability:** The Express server in `src/server.js` was serving static files and API responses without HTTP security headers (such as `X-Content-Type-Options`, `X-Frame-Options`, and `X-XSS-Protection`). This missed an opportunity for defense-in-depth against MIME sniffing, clickjacking, and some forms of Cross-Site Scripting (XSS).
**Learning:** Default Express configurations do not include security headers. They must be explicitly added either manually via middleware or using libraries like `helmet` to establish a secure baseline.
**Prevention:** Added a custom middleware to enforce `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, and `X-XSS-Protection: 1; mode=block` globally before serving any static content or JSON APIs.
