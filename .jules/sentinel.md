...
**Vulnerability:** In `public/app.js`, dynamic content such as `msg`, `item.name`, and `item.path` were being interpolated directly into HTML strings and assigned via `innerHTML` (e.g., in `appendLog`, `appendReasoning`, and `renderTreeItem`). This allowed for XSS if an attacker could control the logs or workspace filenames.
**Learning:** Even internal dashboards must treat data from the backend (like file paths or generated logs) as untrusted when rendering them to the DOM, as filenames or log injections can carry payload structures.
**Prevention:** An `escapeHTML` function was implemented and applied to all untrusted variables before HTML interpolation. For variables embedded inside HTML attributes that contain JavaScript (like inline `onclick` handlers), they were additionally serialized with `JSON.stringify()` prior to escaping to prevent literal breakouts.
